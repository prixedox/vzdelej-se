...
export const TREE_FILE_BY_SUBJECT: Record<string, string> = {
  math: "math-tree.ts",
...
export async function insertTopicIntoTree(
  treePath: string,
  categorySlug: string,
  topicSlug: string,
  displayName: string
...
export async function createTopic({
  ...
  topicSlug,
  displayName,
}: CreateTopicParams): Promise<CreateTopicResult> {
  if (!SLUG_RE.test(categorySlug)) {
    throw new Error(`Invalid category slug shape: ${categorySlug} (expected ${SLUG_RE})`);
  }
  if (!SLUG_RE.test(topicSlug)) {
    throw new Error(`Invalid topic slug shape: ${topicSlug} (expected ${SLUG_RE})`);
  }

  const treeFile = TREE_FILE_BY_SUBJECT[subject];
  if (!treeFile) {
    throw new Error(
      `Unknown subject: ${subject} (expected one of ${Object.keys(TREE_FILE_BY_SUBJECT).join(", ")})`
    );
  }
  ...
  const chapterFile = await createChapter({
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (parts.length !== 3) {
    console.error(`Expected <subject>/<category>/<topic>, got: ${arg}`);
    process.exit(1);
  }
  const [subject, categorySlug, topicSlug] = parts;

  createTopic({ subject, categorySlug, topicSlug, displayName })
    .then(({ treeFile, chapterFile }) => {
      console.log(`✓ Added "${topicSlug}" under category "${categorySlug}" in ${treeFile}`);
      console.log(`✓ Created intro chapter ${chapterFile}`);
      console.log(
